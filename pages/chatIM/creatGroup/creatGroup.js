// pages/chatIM/creatGroup/creatGroup.js
...
Page({
  ...
  data: {
    ...
    friendCard: [],  //关注用户
    friendMap: wx.getStorageSync('followUser'),
  ...
  onLoad: function (options) {
    var follow = wx.getStorageSync('followUser')
    var friendCard = []
    for (let key in follow){
      friendCard.push(follow[key])
    }
    this.setData({
   ...
   */
  onShow() {
    if (this.data.friendCard.length == 0) {
      wx.showToast({
        title: '您还没有关注任何用户',
        icon:'none',
      })
  ...
  createGroup() {
    let accounts = this.data.addList.map(item => item.ticket)
    var options = {
      'GroupId': null,
      'Type': 'Public',
      'Name': '群聊',
    ...
    webimhandler.createBigGroup(options,(err)=>{
      console.log('创建群回调----',err)
      if (err.GroupId){
        wx.navigateTo({
          url: `/pages/chatIM/chat/chat?roomId=${err.GroupId}&type=GROUP`,
        })
      }
    })
