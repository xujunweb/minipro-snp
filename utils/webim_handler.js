...
//默认头像
var friendHeadUrl = '/images/default-icon.png'
//默认群头像
var groupHeadUrl = '/images/advanced.png'
var maxNameLen = 10
var infoMap = {}
...
function onMsgNotify(newMsgList,callback) {
  ...
  for (var j in newMsgList) { //遍历新消息
    ...
    callback && callback(showMsg(newMsg))
  }
}

//获取最新的 C2C 历史消息,用于切换好友聊天，重新拉取好友的聊天消息
function getC2CHistoryMsgs(info,cbOk, cbError) {
      ...
      if (resp.SessionItem && resp.SessionItem.length > 0) {
        ...
        for (var i in resp.SessionItem) {
          ...
          if (type == webim.RECENT_CONTACT_TYPE.C2C) { //私聊
            ...
            var c2cInfo = item;
            if (c2cInfo && c2cInfo.C2cNick) { //从item获取c2c昵称
              sessionNick = c2cInfo.C2cNick; //会话昵称，私聊时为好友昵称，接口暂不支持返回，需要业务自己获取（前提是用户设置过自己的昵称，通过拉取好友资料接口（支持批量拉取）得到）
            } else { //没有找到或者没有设置过
              sessionNick = sessionId; //会话昵称，如果昵称为空，默认将其设成会话id
            }
            if (c2cInfo && c2cInfo.C2cImage) { //从infoMap获取c2c头像
              sessionImage = c2cInfo.C2cImage; //会话头像，私聊时为好友头像，接口暂不支持返回，需要业务自己获取（前提是用户设置过自己的昵称，通过拉取好友资料接口（支持批量拉取）得到）
            } else { //没有找到或者没有设置过
...
function onSendMsg(msg, callback) {
  ...
  var msg = new webim.Msg(selSess, isSend, seq, random, msgTime, loginInfo.identifier, subType, loginInfo.identifierNick, loginInfo.fromAccountHeadurl);
  //解析文本和表情
  var expr = /\[[^[\]]{1,3}\]/mg;
...
function sendGroupLoveMsg() {
  setInterval(quitBigGroup('test'), 10000);
...
function quitBigGroup(ChatRoomId) {
  var options = {
  ...
  webim.quitBigGroup(
    ...
    function(resp) {
      ...
      webim.Log.info('退群成功');
      selSess = null;
      setInterval(createBigGroup("testtest", loginInfo, function() {
        applyJoinBigGroup("testtest"); //加入大群
      }), 20000);
    },
